...
mod tests {
    use super::*;
    use shared_ws::core::{WsFrame, WsPongResult};

    #[test]
    fn public_ping_marks_pong_received_on_op_pong_without_req_id() {
        let mut s = BybitJsonPingPong::public(Duration::from_secs(1), Duration::from_secs(10));
        let _ = s.create_ping().expect("ping");

        let pong = WsFrame::Text(Bytes::from_static(br#"{"op":"pong"}"#));
        match s.handle_inbound(&pong) {
            WsPongResult::PongReceived(_) => {}
            other => panic!("expected pong, got {other:?}"),
        }
    }

    #[test]
    fn private_ping_requires_matching_req_id() {
        let mut s = BybitJsonPingPong::private(Duration::from_secs(1), Duration::from_secs(10));
        let ping = s.create_ping().expect("ping");
        let req_id = match ping {
            WsFrame::Text(bytes) | WsFrame::Binary(bytes) => {
                let v: sonic_rs::Value = sonic_rs::from_slice(bytes.as_ref()).unwrap();
                let id = v.get("reqId").and_then(|v| v.as_str()).unwrap().to_string();
                id
            }
            _ => panic!("unexpected frame type"),
        };

        let pong = WsFrame::Text(Bytes::from(format!(r#"{{"op":"pong","reqId":"{req_id}"}}"#)));
        match s.handle_inbound(&pong) {
            WsPongResult::PongReceived(Some(_)) => {}
            other => panic!("expected pong, got {other:?}"),
        }

        // Non-matching pong should not clear the pending ping.
        let pong2 = WsFrame::Text(Bytes::from_static(br#"{"op":"pong","reqId":"different"}"#));
        match s.handle_inbound(&pong2) {
            WsPongResult::NotPong => {}
            other => panic!("expected not-pong, got {other:?}"),
        }
    }
}
