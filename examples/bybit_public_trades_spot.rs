...
use shared_ws::core::{
    ForwardAllIngress, WsDisconnectAction, WsDisconnectCause, WsEndpointHandler, WsErrorAction,
    WsParseOutcome, WebSocketBufferConfig,
};
use shared_ws::transport::tungstenite::TungsteniteTransport;
use shared_ws::ws::{WebSocketActor, WebSocketActorArgs, WebSocketEvent, WsReconnectStrategy, WsTlsConfig};

use examples_ws::bybit::ping::BybitJsonPingPong;
use examples_ws::endpoints::bybit::{BybitEvent, BybitSubscriptionManager, BybitTopic, BybitTopicHandler};
...
impl WsEndpointHandler for PrintingHandler {
    ...
    fn parse_frame(&mut self, frame: &shared_ws::core::WsFrame) -> Result<WsParseOutcome<Self::Message>, Self::Error> {
            ...
            BybitEvent::Topic { topic, payload } => {
                if topic.0.starts_with("publicTrade.") {
                    for t in parse_public_trade_payload(&payload) {
                        println!(
                            "trade symbol={} side={} price={} qty={} ts_ms={}",
                            t.symbol, t.side, t.price, t.qty, t.ts_ms
                        );
                    }
                } else {
                    println!("topic {} {}", topic.0, String::from_utf8_lossy(payload.as_ref()));
                }
            }
        }
        Ok(())
    ...
    fn handle_server_error(&mut self, code: Option<i32>, message: &str, data: Option<sonic_rs::Value>) -> WsErrorAction {
    ...
    data.iter()
        .filter_map(|row| {
            ...
            let price = row
                ...
                .map(|s| s.to_string())
                .or_else(|| row.get("price").and_then(|v| v.as_f64()).map(|f| f.to_string()))
            ...
            let qty = row
                ...
                .map(|s| s.to_string())
                .or_else(|| row.get("size").and_then(|v| v.as_f64()).map(|f| f.to_string()))
...
async fn main() {
    ...
    let actor = WebSocketActor::spawn(WebSocketActorArgs {
        ...
        reconnect_strategy: ExponentialReconnect::new(Duration::from_secs(1), Duration::from_secs(30)),
        handler,
